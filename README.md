...
- **Fast transcription with `wsi` via local whisper.cpp, or speak to a whisper.cpp server for even faster [network transcription.](./API_TRANSCRIBE.md)**
- **Can select speech input language and translate from the command line with the dedicated `wsiml` script**
- **Can use a [downloaded](https://huggingface.co/Mozilla/whisperfile/tree/main) portable [whisperfile](https://huggingface.co/Mozilla/whisperfile), via command-line flag (set hotkey for `wsi -w`)**
